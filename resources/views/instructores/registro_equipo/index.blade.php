...
<div class="max-w-5xl mx-auto bg-white shadow-md rounded-lg p-6 mt-6" style="background-color:rgb(232, 255, 238)">
    ...
    <div class="mb-4 flex justify-start">
        <a href="{{ route('registro_equipo.create') }}" 
    ...
    <table class="w-full table-auto text-left border">
        ...
        <tbody>
            @forelse ($equipos as $equipo)
                <tr class="border-b"  style="background-color:rgb(217, 255, 226);">
                    ...
                    <td class="px-6 py-4 flex gap-2">
                        <a href="{{ route('registro_equipo.show', $equipo->id) }} " class="text-blue-600 hover:underline">Ver</a>
                        <a href="{{ route('registro_equipo.edit', $equipo->id) }} " class="text-blue-600 hover:underline">Editar</a>
                        <form action="{{ route('registro_equipo.destroy', $equipo->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este equipo?')">
                            @csrf
                            @method('DELETE')
